...
include("../src/SimulationMultiplex.jl")
...
@testset "Model" begin
    @testset "simple test" begin
        graph_game = cycle_graph(5)
        graph_learning = cycle_graph(5)
        model = sim.Model(graph_game, graph_learning, n_game = 4, n_learning = 4, b = 6.0)

        @test model.graph_game == graph_game
        @test model.graph_learning == graph_learning
        @test model.n_game == 4
    ...
    @testset "neighbours' test for simple network" begin
        graph = cycle_graph(9)
        model = sim.Model(graph, graph, n_game = 4, n_learning = 4, b = 6.0)
...
@testset "select_neighbours" begin
    model = sim.Model(cycle_graph(5), cycle_graph(5), n_game = 2, n_learning = 10, b = 6.0)
...
@testset "calc_payoffs!" begin
    model = sim.Model(cycle_graph(5), cycle_graph(5), n_game = 4, n_learning = 2, b = 5.0)
...
@testset "update_agents!" begin
    model = sim.Model(cycle_graph(5), cycle_graph(5), n_game = 4, n_learning = 4, b = 6.0)
...
@testset "make_graph" begin
    ...
    @testset "平均距離 (L)" begin
        N = 1000
        Ls = []
        for _ in 1:100
            g = sim.make_graph(:scale_free_2, N)
            L = mean([sum(gdistances(g, v)) / (N - 1) for v in vertices(g)])
            push!(Ls, L)
        end
        println(mean(Ls))
        @test abs(L - 6.993115155155158) < 0.1

        g = sim.make_graph(:scale_free_4, N)
        L = mean([sum(gdistances(g, v)) / (N - 1) for v in vertices(g)])
        @test abs(L - 4.159375375375375) < 0.1

        g = sim.make_graph(:scale_free_6, N)
        L = mean([sum(gdistances(g, v)) / (N - 1) for v in vertices(g)])
        @test abs(L - 3.469205205205205) < 0.1

        g = sim.make_graph(:scale_free_8, N)
        L = mean([sum(gdistances(g, v)) / (N - 1) for v in vertices(g)])
        @test abs(L - 3.17077077077077) < 0.1
    end

    @testset "scale_free_8" begin
        N = 1000
        g = sim.make_graph(:scale_free_8, N)
        ...
        # 枝数
        @test ne(g) == 3990 == 6 + (N - 4) * 4

        # 平均次数
        @test 2 * ne(g) / nv(g) == 7.98

        # 平均距離 (L)
        L = mean([sum(gdistances(g, v)) / (N - 1) for v in vertices(g)])
        @test abs(L - 3.178) < 0.1

        # クラスター係数 (C)
        C = global_clustering_coefficient(g)
        @test abs(C - 0.026) < 0.0025
    end
